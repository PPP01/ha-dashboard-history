...
> **Am 2026-09-08 ergänzt, nach einer Beobachtung an der Anlage.** Zweimal »Zurück zu dieser Version« auf `dh-probe`, ohne eine einzige Änderung dazwischen, ergaben `v0.0.5` und `v0.0.6` — byte-identisch zu `v0.0.3` und `v0.0.4` und beide mit dem Tagesdatum betitelt. Ursache ist die Vorbelegung: Im einfachen Modus ist das Kästchen angehakt, und nichts fragte, ob der Stand, um den es geht, schon einen Namen trägt. Dritter Fall deshalb, neben den beiden oben: **kein Angebot, wenn eine Version den lebenden Stand schon hält.** Das Kästchen wehrt einen Verlust ab; wo der Stand in der Liste der Versionen steht — und das ist die einzige Liste, die der einfache Modus zeigt —, gibt es keinen abzuwehren. Es ist dieselbe Regel, die H1 vor einer Tagesmarke prüft, einen Modus weiter außen.
>
> **Verglichen wird gegen alle Versionen, nicht gegen die höchstnummerierte** — und darin liegt der Unterschied zur Tagesmarke, wo es umgekehrt begründet ist. Eine Tagesmarke dokumentiert einen *Tag*; sie darf einen Rückgriff nicht verschlucken, bloß weil eine ältere Version denselben Inhalt trägt. Dieses Kästchen dokumentiert nichts, es verhindert einen Verlust — und wo irgendeine Version den Stand hält, ist nichts zu verlieren. Wer trotzdem einen zweiten Namen möchte, hat »Version bis hierher« im erweiterten Modus; die FAQ nennt zwei Versionen auf einem Stand ausdrücklich erlaubt und normal.
>
> **Die Antwort lag schon im Panel.** `history` liefert seit Vorhaben G `matching_versions`, serverseitig über *alle* Versionen des Dashboards per Blob-Kennung gerechnet — auch über die außerhalb des geladenen Fensters. `_versionsMatchingNow` liest sie bereits für den Chip »same state as«. Also kein neues Antwortfeld und kein zusätzlicher Repository-Zugriff je Vorschau. Die Grenze davon ist benannt, und sie ist ein einziges Fenster: zwischen dieser Antwort und dem Dialog. Ein Speichervorgang darin kündigt sich zwar an, aber die Ankündigung wird beiseitegelegt, solange ein Dialog steht. Kostet das einmal das Angebot, geht dennoch nichts verloren — `_keep_the_live_state` schreibt den Stand ohnehin als Eintrag, er hätte nur keinen Namen. Eine Änderung direkt an `.storage/lovelace.<key>` ist dieses Fenster ausdrücklich *nicht*: Home Assistant liest die Datei genau einmal und bedient seine Dashboards danach aus dem Speicher, sieht die Änderung also selbst nicht — nachgelesen in HA 2026.8.3 und in der README unter »When a change is recorded« ausgeführt. Ein frisch im Vorschau-Aufruf berechnetes Feld war die Alternative und wurde verworfen: Es kostet einen Repository-Zugriff je Vorschau und eine zweite Antwortform für einen Vorteil, der nur in diesem Fenster von Millisekunden liegt.
>
> **Statt des Kästchens ein Satz.** Der Absatz, der sonst »What the dashboard holds now is not lost« sagt, nennt in diesem Fall die Version: »already saved as v0.0.5 and v0.0.3, so there is nothing to keep.« Kein neues Markup — dieselbe Stelle, präziser. Ein Angebot, das ohne ein Wort verschwindet, liest sich als Fehler des Werkzeugs.
>
> **Wie es geprüft ist.** In `tests/test_panel_behaviour.py` als dritter Fall des Szenarios `keep_suppressed`, neben »nichts anzuwenden« und »Dashboard wird neu angelegt«: kein Kästchen, kein `keep_as_version` im bestätigenden Aufruf, und der Versionsname steht im Dialog. Dazu die Gegenprobe im gewohnten Fall — sonst hieße »nicht angeboten« womöglich nur, dass der Harness überhaupt kein Angebot zeigen kann. Und einmal am laufenden Panel nachgesehen, weil der Node-Ersatz das eine nicht beantwortet: ob `_matching` im Moment des Dialogs wirklich gefüllt ist. Chrome auf der Testinstanz, `dh-probe` im einfachen Modus, »Zurück zu v0.0.4« geöffnet und mit *Cancel* geschlossen: `_matching` trägt `v0.0.5` und `v0.0.3`, der Kästchen-Block ist `hidden`, der Satz steht wörtlich im Dialog, der Apply-Knopf bleibt, die Konsole ist leer. Eine Falle dabei, die jeder wieder tappt, der das Panel so fährt: `page.js` wertet mit `awaitPromise` aus, und `_restoreState` liefert ein Versprechen, das erst beim Schließen des Dialogs einlöst — direkt abgewartet hält es den eigenen Prüflauf fest. Der Aufruf muss fallen gelassen werden.
...
Erwartet: **`310 passed, 3 skipped`** (dreizehn mehr), Integrationsprüfungen grün. Am Panel beide Wege gehen:
