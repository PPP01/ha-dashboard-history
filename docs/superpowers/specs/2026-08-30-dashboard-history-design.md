...
20. **Das Panel wird schmal bedienbar — gemessen an sich selbst, nicht am Fenster.** *(Nachgetragen am 2026-09-17, auf Wunsch des Nutzers. Anlass war ein Screenshot vom Handy, kein Review.)*
    ...
    **Der Befund, in den Worten dessen, der es versucht hat:** »Am Handy ist es nahezu unbedienbar.« Drei Beobachtungen, und jede hat eine Ursache im Code, keine im Geschmack:
    ...
    4. **Und eine vierte, die erst sichtbar wurde, als die ersten drei behoben waren:** `.verhead`, der Kopf einer Versionszeile, ist ebenfalls eine Flexzeile ohne `flex-wrap`. *(Nachgetragen am 2026-09-17 — siehe Festlegung 8.)*
    ...
    **Festlegung 5: Der Balken darf umbrechen, statt Inhalte zu verlieren.** Im schmalen Band steht in der ersten Zeile `☰ ‹ Titel ⟳` und in der zweiten der Simple/Advanced-Schalter. Die Alternative wäre gewesen, den Schalter für schmale Schirme wegzulassen oder ihn in die Inhaltsspalte zu verschieben; das Erste nimmt dem Handy einen der beiden Modi, das Zweite verlangt entweder doppeltes Markup mit kollidierenden Radio-Namen oder ein Messen in JavaScript, das Festlegung 4 gerade vermeidet. *(Am 2026-09-17 nach den ersten Aufnahmen ergänzt: **Im Balken steht dann genau eine Beschriftung, und welche, entscheidet die Spalte.** Beide nebeneinander machten ihn dreizeilig — »Dashboard History« füllte die erste Zeile allein, der Dashboard-Name rutschte in die zweite, und der Reload-Knopf strandete am rechten Rand einer sonst leeren Zeile. 124 px Balken für vier Bedienelemente, gegen 86 px mit einer Beschriftung. Gesehen auf einem Bild; die Messung meldete die Höhe und schwieg über die Form.

    Im **Verlauf** ist es der Dashboard-Name: Home Assistants Seitenleiste und der Seitentitel sagen ohnehin »Dashboard History«, und was auf einem Handy fehlt, ist die Angabe, welches Dashboard man ansieht. Auf der **Liste** ist es umgekehrt, und das kam von jemandem, der es benutzt hat: Dort ist noch nichts gewählt — die erste Zeile ist vorausgewählt, aber man steht vor der Wahl und nicht vor einem Ergebnis. Ein Dashboard-Name über einer Liste von Dashboards beantwortet eine Frage, die noch niemand gestellt hat.)*

    **Damit fällt die feste `height: 56px` des Balkens** und mit ihr `.layout { height: calc(100% - 56px) }`. Die Zeile ist ohnehin schon wirkungslos — der lange Kommentar direkt darunter sagt, warum: Der Host misst sich an seinem Inhalt, nicht am Fenster, und keine der beiden Spalten hält je mehr, als hineinpasst. Ersetzt wird sie durch eine Flex-Spalte auf dem Host. Das ist eine Aufräumarbeit, aber keine beiläufige: Ohne sie ist ein umbrechender Balken nicht sicher zu haben.
    ...
    **Festlegung 8: Keine Flexzeile im Inhalt bleibt auf `nowrap`.** *(Nachgetragen am 2026-09-17, gefunden beim Abnehmen von Vorhaben K, Aufgabe 5 — von dem Kriterium, das drei Absätze weiter oben geschärft wurde. Es ist der erste Fund dieses Kriteriums und zugleich der Beleg, dass es taugt.)*

    Der Befund oben nennt drei Ursachen für den waagerechten Überlauf. Es gibt eine **vierte**, und sie ist dieselbe Krankheit wie die dritte an einer anderen Stelle: `details.ver > summary .verhead` — der Kopf einer Versionszeile mit Titel, Stift und Papierkorb — ist eine Flexzeile ohne `flex-wrap`. Bei 390 px gemessen, erweiterter Modus: `.verhead` verlangte 424 px und hatte 316, der Papierkorb lag 76 px jenseits der Inhaltsspalte, und `.main` stand 380 px breit um 456 px Inhalt.

    Dass es erst hier auffiel, liegt an der Reihenfolge: Solange die Seitenspalte 280 px festhielt, war die Inhaltsspalte auf einem Handy ohnehin zu schmal für alles. Erst als Aufgabe 5 ihr die volle Breite gab, blieb genau diese eine Zeile übrig — und `docScrolls` hätte auch sie nicht gezeigt, weil `.main` den Überlauf schluckt.

    **Die Festlegung ist deshalb allgemeiner als der Fund:** Im schmalen Band bricht **jede** Flexzeile der Inhaltsspalte um, statt Breite zu verlangen, die es nicht gibt. Heute sind das `.search` (Festlegung 6) und `.verhead`; wer eine dritte hinzufügt, gibt ihr `flex-wrap: wrap` gleich mit. Gemessen wirkt es: `.verhead` 316 gegen 316, `.main` 380 gegen 380.

    **Eine Ausnahme, und sie steht schon da:** der technische Diff. `pre` trägt `overflow-x: auto` und scrollt in sich — bei derselben Messung 407 gegen 284 —, und das ist ausdrücklich erlaubt (siehe »Was diese Entscheidung nicht ist«). Monospace-Zeilen umzubrechen hieße, einen Diff unlesbar zu machen, um eine Regel zu erfüllen.
